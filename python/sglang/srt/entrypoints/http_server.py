...
def _wait_and_warmup(
    ...
    # Schema-5 DWS-WSL owns a batch-one warmup/probe barrier before readiness.
    v2_probed = False
    if server_args.dllm_algorithm and server_args.my_sjf_cost_model.lower() == "dws-wsl":
        try:
            from sglang.srt.dllm.my_code.probe_scale import start_serving_probes
            v2_probed = start_serving_probes(server_args)
        except Exception:
            logger.exception("DWS-WSL scale initialization failed; refusing readiness")
            kill_process_tree(os.getpid())
            return
    # Send a warmup request
    if not v2_probed and not server_args.skip_server_warmup:
...
def launch_server(
    ...
    """
    # Keep v2 admission closed until the HTTP warmup/probe coordinator publishes.
    server_args._dllm_cost_probe_http_startup = True
    # Launch subprocesses
    (
        tokenizer_manager,
        template_manager,
