...
class ReqDllmMixin:
    def init_diffusion_llm(self: Req, dllm_config: DllmConfig):
        ...
        # Populated lazily by the calibrated DWS scheduler.  Keeping the cache
        # on the request avoids re-walking the full denoising-depth curve every
        # time the same candidate is compared or written to the SJF log.
        self.my_calibrated_cost_cache = None
