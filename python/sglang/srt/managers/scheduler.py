...
class Scheduler(
    ...
    def process_input_requests(self, recv_reqs: List):
        ...
        for recv_req in recv_reqs:
            ...
            if is_health_check_generate_req(recv_req) and (
                getattr(self, "_my_cost_probe_window", None)
                or getattr(self, "_my_cost_probe_state", "") in {"pending", "failed"}
    ...
    def handle_generate_request(
        ...
        self._cost_probe_accept_request(req)
        if req.rid.startswith("__dllm_cost_probe__") and not getattr(req, "my_cost_probe", False):
            prepare_abort(req, "probe window cancelled or RID not registered", status_code=HTTPStatus.BAD_REQUEST)
            self.stream_output([req], req.return_logprob)
            return
        req.my_cost_health_check = is_health_check_generate_req(recv_req)
        profile = getattr(self.dllm_config, "my_cost_profile", None)
        if getattr(profile, "is_v5", False) and not getattr(req, "my_cost_probe", False) and not req.my_cost_health_check:
            if req.sampling_params.max_new_tokens != profile.max_new_tokens:
                prepare_abort(req, "max_new_tokens_mismatch: DWS-WSL trajectory contract requires " + str(profile.max_new_tokens),
    ...
    def get_internal_state(self, recv_req: GetInternalStateReq):
        ...
        if getattr(self, "_my_cost_runtime_signature", None) is not None:
            ret["dllm_cost_runtime_signature"] = self._my_cost_runtime_signature
        cfg = getattr(self, "dllm_config", None)
        if getattr(self, "_my_cost_shape_signature", None) is not None:
            ret["dllm_cost_shape_signature"] = self._my_cost_shape_signature
        ret.update(self._cost_probe_status())
        profile = getattr(cfg, "my_cost_profile", None)
        if profile is not None:
            ret["dllm_cost_profile_id"] = profile.profile_id
            ret["dllm_cost_model"] = cfg.my_sjf_cost_model
            if getattr(profile, "is_v5", False):
                ret["dllm_cost_shape_id"] = profile.shape_id
                ret["dllm_cost_scale_id"] = profile.scale_id if self._my_cost_probe_state not in {"pending", "failed"} else ""
                ret["dllm_cost_scale"] = profile.payload["scale"]
    ...
    def set_internal_state(self, recv_req: SetInternalStateReq):
        ...
        if "dllm_cost_probe_control" in server_args_dict:
            try:
                if len(server_args_dict) != 1:
                    raise ValueError("probe control cannot be combined with server argument updates")
                self._cost_probe_control(server_args_dict["dllm_cost_probe_control"])
                self._my_cost_probe_control_error = None
            except Exception as exc:
                self._my_cost_probe_control_error = str(exc)
                logger.warning("DWS-WSL probe control rejected: %s", exc)
            return SetInternalStateReqOutput(updated=self._my_cost_probe_control_error is None, server_args={})
