    ...
    def _handle_batch_output(
        ...
        for i, rid in enumerate(recv_obj.rids):
            ...
            if state.finished:
                ...
                # my_code: per-request server-side latency metrics (default off)
                if getattr(self.server_args, "my_metrics_log", "") and not rid.startswith("__dllm_cost_probe__"):
                    from sglang.srt.dllm.my_code.metrics import log_request as _my_log
                    ...
                    # dLLM server-side TTFT: prefer the scheduler-stamped first-demask
                    # time (true time-to-first-token) over the tokenizer's first-output
                    # time (= completion time for non-streaming requests).
                    _sched_ts = (
                        recv_obj.time_stats[i]
                        if recv_obj.time_stats is not None
                        else None
                    )
                    _actual_denoise_steps = (
                        int(getattr(_sched_ts, "my_actual_denoise_steps", 0))
                        if _sched_ts is not None
                        else None
                    )
                    if _actual_denoise_steps is not None:
                        # out_dict retains this meta_info object, so the online
                        # benchmark client receives the same per-request truth.
                        meta_info["actual_denoise_steps"] = _actual_denoise_steps
                    _ftt = (
                        _sched_ts.my_first_token_time
                        if (
                            _sched_ts is not None
                            and getattr(_sched_ts, "my_first_token_time", 0.0) > 0.0
                        )
                        else state.time_stats.first_token_time
                    )
                    _dst = (
                        getattr(_sched_ts, "my_decode_start_time", 0.0)
                        if _sched_ts is not None
                        else 0.0
                    )
                    # Short prompts can enter dLLM decode immediately; for
                    # these requests the first demask is the best available
                    # decode-forward boundary.
                    if _dst <= 0.0:
                        _dst = _ftt
                    _my_log(
                        self.server_args.my_metrics_log,
                        rid=rid,
                        created_time=state.time_stats.created_time,
                        first_token_time=_ftt,
                        finished_time=state.time_stats.finished_time,
                        completion_tokens=_ct,
                        prompt_tokens=meta_info.get("prompt_tokens"),
                        finish_reason=(
                            recv_obj.finished_reasons[i].get("type")
                            if isinstance(recv_obj.finished_reasons[i], dict)
                            else str(recv_obj.finished_reasons[i])
                        ),
                        extra={
                            "actual_denoise_steps": _actual_denoise_steps,
                            "request_start_ts": convert_time_to_realtime(
                                state.time_stats.created_time
                            ),
                            "decode_start_ts": convert_time_to_realtime(_dst),
                            "decode_end_ts": convert_time_to_realtime(
                                state.time_stats.finished_time
                            ),
                            "decode_start_monotonic_s": _dst,
                            "decode_end_monotonic_s": state.time_stats.finished_time,
                        },
                    )
