...
class SchedulerReqTimeStats(ReqTimeStatsBase):
    ...
    decode_ct: int = 0
    # my_*: dLLM experiment metrics shipped to the tokenizer when
    # --my-metrics-log is enabled.
    # - my_first_token_time: wall-clock of the first demask/committed token.
    # - my_decode_start_time: wall-clock of the first dLLM decode forward.
    # - my_actual_denoise_steps: sum of the real per-output-block s_b values.
    my_first_token_time: float = 0.0
    my_decode_start_time: float = 0.0
    my_actual_denoise_steps: int = 0
    my_enable_first_token_time_export: bool = False
    ...
    def __getstate__(self) -> object:
        ...
        if not self.enable_metrics and not self.my_enable_first_token_time_export:
            return {}
        ...
        if self.my_enable_first_token_time_export:
            state["my_enable_first_token_time_export"] = True
            state["my_actual_denoise_steps"] = int(self.my_actual_denoise_steps)
            if self.my_first_token_time > 0.0:
                state["my_first_token_time"] = self.my_first_token_time
            if self.my_decode_start_time > 0.0:
                state["my_decode_start_time"] = self.my_decode_start_time
        return state
