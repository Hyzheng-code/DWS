...
class ServerArgs:
    ...
    # Diffusion LLM and DWS non-preemptive scheduling
    ...
    my_sjf_promote_aging: Optional[float] = None
    ...
    def add_cli_args(parser: argparse.ArgumentParser):
        ...
        parser.add_argument(
            "--my-metrics-log",
            type=str,
            default=ServerArgs.my_metrics_log,
            help="Write per-request server-side latency metrics (TTFT / e2e / "
            "per-token) as JSONL to this path. Empty disables (default).",
        )
        parser.add_argument(
            "--my-sjf",
            action="store_true",
            default=ServerArgs.my_sjf,
            help="Enable my predicted-workload non-preemptive SJF scheduling for "
            "dLLM (scored prefill pool + sticky shortest-job-first decode "
            "admission). Default off.",
        )
        parser.add_argument(
            "--my-sjf-dws",
            action="store_true",
            default=ServerArgs.my_sjf_dws,
            help='Enable phase-aware DWS-WSL costs; requires --my-sjf, a DWS predictor and --my-sjf-cost-profile.',
        )
        parser.add_argument(
            "--my-sjf-cost-model",
            type=str.lower,
            choices=("auto", "dws-wsl"),
            default=ServerArgs.my_sjf_cost_model,
            help='DWS cost model. auto selects dws-wsl when --my-sjf --my-sjf-dws are enabled.',
        )
        parser.add_argument("--my-cost-probe", choices=("auto", "force", "off"), default=ServerArgs.my_cost_probe,
                            help="DWS-WSL full cost fit: reuse a matching WLS profile or profile automatically; force adds new traces; off explicitly uses the file (including legacy/stale profiles).")
        parser.add_argument("--my-cost-probe-refresh", choices=("off", "idle"), default=ServerArgs.my_cost_probe_refresh,
                            help="Opt-in idle full W/delta refit with cumulative statistics; yields to requests and caps probe GPU share at 0.5%%.")
        parser.add_argument("--my-cost-probe-timeout", type=float, default=ServerArgs.my_cost_probe_timeout,
                            help="Seconds per probe HTTP request; 0 disables timeout. Failed probes retry once.")
        parser.add_argument("--my-cost-probe-prompts", type=str, default=ServerArgs.my_cost_probe_prompts,
                            help="Optional probe suite JSONL for converted schema-4 profiles.")
        parser.add_argument(
            "--my-sjf-cost-profile", type=str, default=ServerArgs.my_sjf_cost_profile,
            help='External DWS-WSL profile: managed version/family directory or compatible JSON file.',
        )
        parser.add_argument(
            "--my-sjf-pool",
            type=int,
            default=ServerArgs.my_sjf_pool,
            help="dLLM SJF scored-prefill-pool size (max requests prefilled+scored "
            "ahead of the decode set). 0 = 2 * max_running_requests.",
        )
        parser.add_argument(
            "--my-sjf-aging",
            type=float,
            default=ServerArgs.my_sjf_aging,
            help="Fractional cost discount per full 30 seconds in the waiting "
            "queue: score * (1 - discount)^floor(wait/30). Default 0.1 "
            "implements the paper's 10%% discount. Must be in [0, 1); 0 disables "
            "aging. Running requests remain non-preemptive.",
        )
        parser.add_argument(
            "--my-sjf-log",
            type=str,
            default=ServerArgs.my_sjf_log,
            help="Write dLLM SJF decode-admission decisions as JSONL to this path. "
            "Empty disables (default).",
        )
        parser.add_argument(
            "--my-sjf-prefill-quota",
            type=int,
            default=ServerArgs.my_sjf_prefill_quota,
            help="dLLM SJF prefill exploration quota in blocks per round (A). "
            "0 = strict prefill-priority (original behavior). When >0, prefill "
            "rounds are interleaved at ~quota/max_running_requests fraction so a "
            "prefill burst no longer blocks ready short decodes. Default 2 "
            "(~quota/B prefill rounds; keep below the natural prompt/(prompt+out) "
            "block ratio to favor decode). Only active together with --my-sjf.",
        )
        parser.add_argument(
            "--my-sjf-prefill-order",
            action="store_true",
            default=ServerArgs.my_sjf_prefill_order,
            help="dLLM SJF: pull/prefill shortest-prompt requests first from the "
            "backlog (B), so likely-short jobs reach the scored pool (and decode) "
            "soonest. Default off (FIFO).",
        )
        parser.add_argument(
            "--my-sjf-ready-low",
            type=int,
            default=ServerArgs.my_sjf_ready_low,
            help="Adaptive SJF low watermark for prefetched, non-running decode "
            "candidates. 0 = max(B/4, 1).",
        )
        parser.add_argument(
            "--my-sjf-ready-high",
            type=int,
            default=ServerArgs.my_sjf_ready_high,
            help="Adaptive SJF target watermark when forming a closed prefill "
            "cohort. 0 = max(B/2, ready-low).",
        )
        parser.add_argument(
            "--my-sjf-prefill-pause-ratio",
            type=float,
            default=ServerArgs.my_sjf_prefill_pause_ratio,
            help="Adaptive SJF maximum continuous prefill pause as a multiple of "
            "the measured decode-round EWMA. A capped cohort resumes after one "
            "decode round. Default 1.0.",
        )
        parser.add_argument(
            "--my-sjf-promote-aging",
            type=float,
            default=ServerArgs.my_sjf_promote_aging,
            help="Override the fractional cost discount per full 30 seconds "
            "for SJF promotion and optional prefill ordering. Defaults to "
            "--my-sjf-aging (0.1). Must be in [0, 1); 0 disables aging for "
            "these queues. Requires --my-sjf.",
        )
        parser.add_argument(
            "--my-occupancy-log",
            type=str,
            default=ServerArgs.my_occupancy_log,
            help="Write per-forward-round dLLM batch occupancy (prefill/decode "
            "round, #reqs forwarded, active-set/backlog sizes, KV usage) as JSONL "
            "to this path, for FCFS and SJF. Diagnostic for "
            "throughput-tax analysis. Empty disables (default).",
        )
