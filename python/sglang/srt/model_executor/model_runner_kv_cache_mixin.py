...
class ModelRunnerKVCacheMixin:
    ...
    def _init_pools(self: ModelRunner):
        ...
        # dLLM SJF prefills a larger "scored candidate pool" (--my-sjf-pool) than
        # it decodes; every candidate holds a request slot until admitted/finished.
        # Size the request-slot pool to that pool, SEPARATELY from the decode batch
        # (max_running_requests still caps the running/decode set, the cuda-graph
        # batch sizes, and KV sizing). max_num_reqs is only used to size
        # req_to_token_pool below, so this is the single safe place to widen it.
        # Each extra slot is just max_context_len int32 indices -> cheap.
        _sa = self.server_args
        if getattr(_sa, "my_sjf", False):
            _pool = (
                _sa.my_sjf_pool
                if (_sa.my_sjf_pool and _sa.my_sjf_pool > 0)
                else 2 * self.max_running_requests
            )
            # pool candidates + a decode batch can transiently coexist -> + margin
            max_num_reqs = max(max_num_reqs, int(_pool) + self.max_running_requests)
            logger.info(
                "[my_sjf] req_to_token_pool sized to %d (pool=%d, decode batch=%d)",
                max_num_reqs, int(_pool), self.max_running_requests,
            )
